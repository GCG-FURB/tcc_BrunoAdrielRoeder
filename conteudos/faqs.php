...
class FAQ
{
    ...
    private function listarAcoes($secao, $subsecao, $item, $categoria, $ativas)
    {
        ...
        $util = new Utilitario();
        require_once 'conteudos/faqs_categorias.php';
        $faq_cat = new FAQCategoria();
      
        $opcoes_1= array();
        $opcao= array();
        $opcao['indice']= "1";
        $opcao['link']= "index.php?secao=".$secao."&sub=".$subsecao."&it=".$item."&ct=".$categoria."&at=1";
        if ($ativas == '1') {
        ...
        $opcao= array();
        $opcao['indice']= "2";
        $opcao['link']= "index.php?secao=".$secao."&sub=".$subsecao."&it=".$item."&ct=".$categoria."&at=0";
        if ($ativas == '0') {
            $opcao['selecionado'] = ' selected=\"selected\" ';
        } else {
            $opcao['selecionado'] = '';
        ...
        $opcao= array();
        $opcao['indice']= "3";
        $opcao['link']= "index.php?secao=".$secao."&sub=".$subsecao."&it=".$item."&ct=".$categoria."&at=2";
        if ($ativas == '2') {
            $opcao['selecionado'] = ' selected=\"selected\" ';
        } else {
            $opcao['selecionado'] = '';
    ...
    private function listarItens($secao, $subsecao, $item, $categoria, $ativas)
    {
        ...
        foreach ($itens as $it) {
            ...
            if ($nr_caracteres_abreviar_pergunta_faq_restrita > 0) {
                echo $util->abreviar($it['ds_pergunta'], $nr_caracteres_abreviar_pergunta_faq_restrita);
            } else {
                echo $it['ds_pergunta'];
            }
            echo "</b><br />";
            if ($nr_caracteres_abreviar_resposta_faq_restrita > 0) {
                echo $util->abreviar($it['ds_resposta'], $nr_caracteres_abreviar_resposta_faq_restrita);
            } else {
                echo $it['ds_resposta'];
            }
            echo "</td>\n";
