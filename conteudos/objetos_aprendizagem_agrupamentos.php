...
class ObjetoAprendizagemAgrupamento
{
    ...
    public function controleExibicao($secao, $subsecao, $item, $cd_objeto_aprendizagem, $acao)
    {
        ...
        if (isset($_GET['atv'])) {
            $ativas = addslashes($_GET['atv']);
        } else {
            $ativas = 1;
        }
        if (isset($_GET['cdc'])) {
            $codigo = addslashes($_GET['cdc']);
        } else {
            $codigo = '';
        }
        if (isset($_GET['tp'])) {
    ...
    private function listarAcoes($secao, $subsecao, $item, $cd_objeto_aprendizagem, $ativas)
    {
        ...
        $opcao= array();
        $opcao['indice']= "1";
        $opcao['link']= $_SESSION['life_link_completo']."index.php?secao=".$secao."&sub=".$subsecao."&it=".$item."&fc=agrupamento&cd=".$cd_objeto_aprendizagem."&atv=1";
        if ($ativas == '1') {
        ...
        $opcao= array();
        $opcao['indice']= "2";
        $opcao['link']= $_SESSION['life_link_completo']."index.php?secao=".$secao."&sub=".$subsecao."&it=".$item."&fc=agrupamento&cd=".$cd_objeto_aprendizagem."&atv=0";
        if ($ativas == '0') {
            $opcao['selecionado'] = ' selected=\"selected\" ';
        } else {
            $opcao['selecionado'] = '';
        ...
        $opcao= array();
        $opcao['indice']= "3";
        $opcao['link']= $_SESSION['life_link_completo']."index.php?secao=".$secao."&sub=".$subsecao."&it=".$item."&fc=agrupamento&cd=".$cd_objeto_aprendizagem."&atv=2";
        if ($ativas == '2') {
            $opcao['selecionado'] = ' selected=\"selected\" ';
        } else {
            $opcao['selecionado'] = '';
    ...
    public function retornaListaAgrupamentosObjetoAprendizagem($cd_objeto_aprendizagem)
    {
        ...
        if (count($agrupamentos) > 0) {
            ...
            foreach ($agrupamentos as $ag) {
                ...
                echo "    <div class=\"divUmElementoCapaTopo\" style=\"background-color:".$item['ds_cor']."; color: #004B80;\">\n";
