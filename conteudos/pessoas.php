...
class Pessoa
{
    ...
    private function listarAcoes($secao, $subsecao, $item, $ativas, $categoria, $inicial)
    {
        ...
        $util = new Utilitario();
        require_once 'conteudos/categorias_usuarios.php';
        $cat = new CategoriaUsuario();
        ...
        $opcao['indice']= $i;
        $i+=1;
        $opcao['link']= "index.php?secao=".$secao."&sub=".$subsecao."&it=".$item."&ct=".$categoria."&in=".$inicial."&at=1";
        if ($ativas == '1') {
        ...
        $opcao['indice']= $i;
        $i+=1;
        $opcao['link']= "index.php?secao=".$secao."&sub=".$subsecao."&it=".$item."&ct=".$categoria."&in=".$inicial."&at=0";
        if ($ativas == '0') {
            $opcao['selecionado'] = ' selected=\"selected\" ';
        } else {
            $opcao['selecionado'] = '';
        ...
        $opcao['indice']= $i;
        $i+=1;
        $opcao['link']= "index.php?secao=".$secao."&sub=".$subsecao."&it=".$item."&ct=".$categoria."&in=".$inicial."&at=2";
        if ($ativas == '2') {
            $opcao['selecionado'] = ' selected=\"selected\" ';
        } else {
            $opcao['selecionado'] = '';
        ...
        if (count($categorias) > 0) {
            foreach ($categorias as $c) {
                ...
                if ($c['cd_categoria_usuario'] == $categoria) {
                    $opcao['selecionado'] = ' selected=\"selected\" ';
                } else {
                    $opcao['selecionado'] = '';
                }
        ...
        foreach ($opcoes_2 as $op) {
            $nome = 'comandos_filtros_2_'.$op['indice'];
            $util->campoHidden($nome, $op['link']);
        }
        ...
        if (isset($_SESSION['life_c_pesquisando'])) {
            ...
            if (isset($_SESSION['life_c_termo_pessoa'])) {
                $termo = $_SESSION['life_c_termo_pessoa'];
            } else {
                $_SESSION['life_c_termo_pessoa'] = '';
                $termo = $_SESSION['life_c_termo_pessoa'];
            }
            if (isset($_SESSION['life_c_campo_pessoa'])) {
                $campo = $_SESSION['life_c_campo_pessoa'];
            } else {
                $_SESSION['life_c_campo_pessoa'] = 'N';
                $campo = $_SESSION['life_c_campo_pessoa'];
            }
        }
        echo "  <select name=\"comandos_filtros_1\" id=\"comandos_filtros_1\" class=\"fontComandosFiltros\" onChange=\"navegar(1);\" title=\"Filtro de status\">\n";
        foreach ($opcoes_1 as $op) {
            echo "    <option value=\"".$op['indice']."\" ".$op['selecionado'].">".$op['descricao']."&nbsp;</option>\n";
        }
        echo "  </select>\n";
        echo "  <select name=\"comandos_filtros_2\" id=\"comandos_filtros_2\" class=\"fontComandosFiltros\" onChange=\"navegar(2);\" title=\"Filtro de categoria\">\n";
        foreach ($opcoes_2 as $op) {
            echo "    <option value=\"".$op['indice']."\" ".$op['selecionado'].">".$op['descricao']."&nbsp;</option>\n";
        }
        echo "  </select>\n";
    ...
    {
        require_once 'conteudos/contatos.php';
        $con = new Contato();
        $termo = $_SESSION['life_c_termo_pessoa'];
        $campo = $_SESSION['life_c_campo_pessoa'];
        ...
        if ($inicial != '0') {
            $mensagem.= " - com nome iniciado em '".$inicial."'";
        } else {
            $mensagem.= " - com nome iniciado com qualquer letra";
        }
    ...
    {
        require_once 'conteudos/areas_formacao.php';
        $are_for = new AreaFormacao();
        require_once 'conteudos/enderecos.php';
        $end = new Endereco();
        require_once 'conteudos/contatos.php';
        $con = new Contato();
        require_once 'usuarios/usuarios.php';
...
//***************CATEGORIA DO USUARIO ******************************************
    ...
    public function salvarCadastroAlteracaoCategoria()
    {
        ...
        require_once 'conteudos/categorias_usuarios.php';
        $cat = new CategoriaUsuario();
        require_once 'conteudos/email.php';
        $email = new Email();
...
//***************EXIBICAO PUBLICA***********************************************
    ...
    public function imprimeFormularioCadastroProprioPessoa($ds_username, $ds_senha, $nm_pessoa, $nr_cpf)
    {
        ...
        echo "              <input type=\"text\" maxlength=\"100\" class=\"fontConteudoCampoTextHint\" style=\"width:100%;\" name=\"nm_pessoa\" id=\"nm_pessoa\" placeholder=\"Nome completo\" title=\"Informe seu Nome completo\" value=\"".$nm_pessoa."\" onKeyPress=\"ehValidar('1'); ehValido('eh_nome_valido', '0');\" onBlur=\"ehNomeCompleto();\"><br />\n";
        echo "              <input type=\"text\" maxlength=\"100\" class=\"fontConteudoCampoTextHint\" style=\"width:100%;\" name=\"ds_username\" id=\"ds_username\" placeholder=\"E-mail\" title=\"Informe seu E-mail\" value=\"".$ds_username."\" onKeyPress=\"ehValidar('1'); ehValido('eh_username_valido', '0');\" onBlur=\"ehEmailValido();\"><br />\n";
    ...
    public function retornoCadastro()
    {
        ...
        if (isset($_SESSION['life_cadastro_proprio_usuario_sucesso'])) {
            ...
            echo "<br />\n";
            echo "    <p class=\"fontComandosCentralizados\">\n";
            echo "      <a href=\"".$_SESSION['life_link_completo']."index.php?secao=17&sub=20&it=32\"><input type=\"button\" class=\"botao\" value=\"Sim\" title=\"Realizar cadastro completo agora\" border=\"0\"></a>\n";
...
//******************************SENHA*******************************************
    ...
    public function imprimeFormularioEsqueceuSenha($ds_username)
    {
        ...
        echo "      </p>\n";
        echo "      <p class=\"fontConteudoLogin\" style=\"text-align:center;\">\n";
        echo "  		  <input type=\"button\" class=\"botao\" value=\"Envie a senha\" title=\"Envie a senha\" onClick=\"validaEnvioSenha(cadastro);\" onKeyPress=\"validaEnvioSenha(cadastro);\">\n";
        echo "      </p>\n";
        echo "  </form>\n";
        echo "  </div>\n";
    }
