<?php
  class FAQCategoria {
    ...
    public function listarAcoes($secao, $subsecao, $item, $ativas) {
      ...
      echo "  <a href=\"index.php?secao=".$secao."&sub=".$subsecao."&it=".$item."&at=".$ativas."&acao=cadastrar\"><img src=\"icones/novo.png\" alt=\"Nova seção de perguntas para a seção como usar\" title=\"Nova seção de perguntas para a seção como usar\" border=\"0\"></a> \n";
    ...
    private function imprimeFormularioCadastro($link, $cd_categoria, $nm_categoria, $ds_categoria, $nr_ordem, $eh_ativo) {
      ...
      $util->linhaUmCampoText(1, 'Seção ', 'nm_categoria', 100, 100, $nm_categoria);
    ...
    public function salvarCadastroAlteracao() {
      ...
      if ($cd_categoria > 0) {
        ...
        if ($this->insereCategoria($nm_categoria, $ds_categoria, $nr_ordem, $eh_ativo, $lk_seo)) {
          echo "<p class=\"fontConteudoSucesso\">Seção cadastrada com sucesso!</p>\n";
        } else {
          echo "<p class=\"fontConteudoAlerta\">Problemas no cadastro da seção!</p>\n";
        }
      }
    } 
    ...
    public function alterarStatus($cd_categoria) {
      ...
      if ($this->alteraCategoria($cd_categoria, $nm_categoria, $ds_categoria, $nr_ordem, $eh_ativo, $lk_seo)) {
        echo "<p class=\"fontConteudoSucesso\">Status da seção alterado com sucesso!</p>\n";
      } else {
        echo "<p class=\"fontConteudoSucesso\">Problemas ao alterar status da seção!</p>\n";
      }                                                                                               
    }   
    ...
    public function retornaSeletorCategorias($cd_categoria) {
      ...
      $opcoes= array();
      $opcao= array();      $opcao[]= '0';                      $opcao[]= 'Selecione uma seção';      $opcoes[]= $opcao;
      foreach ($itens as $it) {
        $opcao= array();    $opcao[]= $it['cd_categoria'];      $opcao[]= $it['nm_categoria'];            $opcoes[]= $opcao;
      }
      $util->linhaSeletor('Seção *', 'cd_categoria', $cd_categoria, $opcoes, '100');
    }
...
//**************BANCO DE DADOS**************************************************    
    public function selectCategorias($eh_ativo) {
      ...
      $sql.= "ORDER BY nr_ordem, nm_categoria";
