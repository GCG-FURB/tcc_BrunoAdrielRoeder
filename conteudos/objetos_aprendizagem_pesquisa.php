<?php
  class ObjetoAprendizagemPesquisa {
    ...
    public function listarOpcoesPesquisaCapa($secao, $subsecao, $item) {
      ...
      if (isset($_SESSION['life_pesquisa_secao']))    {      $secao = $_SESSION['life_pesquisa_secao'];          }
      if (isset($_SESSION['life_pesquisa_subsecao'])) {      $subsecao = $_SESSION['life_pesquisa_subsecao'];    }
      if (isset($_SESSION['life_pesquisa_item']))     {      $item = $_SESSION['life_pesquisa_item'];            }

      if (isset($_SESSION['life_c_termo']))           {     $termo = $_SESSION['life_c_termo'];                      } else {        $_SESSION['life_c_termo'] = '';               $termo = '';              }
      if (isset($_SESSION['life_c_campos']))          {     $campos = $_SESSION['life_c_campos'];                    } else {        $_SESSION['life_c_campos'] = '';              $campos = array();        }
      if (isset($_SESSION['life_c_eh_proprietario'])) {     $eh_proprietario = $_SESSION['life_c_eh_proprietario'];  } else {        $_SESSION['life_c_eh_proprietario'] = '1';    $eh_proprietario = '1';   }

      echo "  <p style=\"width:97%; text-align:right;\"><a href=\"javascript:void()\" onClick=\"ocultarPesquisa(event);\" onKeyPress=\"ocultarPesquisa(event);\"><img src=\"".$_SESSION['life_link_completo']."icones/fechar_formulario.png\" alt=\"Fechar formulário de pesquisa\" title=\"Fechar formulário de pesquisa\" border=\"0\" onMouseOver=\"this.style.cursor='pointer';\" onFocus=\"this.style.cursor='pointer';\"/></a></p>\n";
    ...
    public function pesquisarSimples($secao, $subsecao, $item, $ativas, $ordem) {
      ...
      if (isset($_POST['cd_formato_objeto'])) {
        $campo['campo'] = 'formato';                  $campo['cd_campo'] = addslashes($_POST['cd_formato_objeto']);                     $campos[] = $campo;
      }
...
//*******************EXIBICAO PUBLICA*******************************************
    ...
    public function controleExibicaoPublica($pagina, $lista_paginas) {
      ...
      if (isset($_POST['cd_formato_objeto']))       {        $campo['campo'] = 'formato';               $campo['cd_campo'] = addslashes($_POST['cd_formato_objeto']);           $campos[] = $campo;      }
      ...
      if (isset($_POST['secao']))    {       $secao = addslashes($_POST['secao']);        $_SESSION['life_pesquisa_secao'] = $secao;         } elseif (isset($_SESSION['life_pesquisa_secao']))    {      $secao = $_SESSION['life_pesquisa_secao'];          }
      if (isset($_POST['subsecao'])) {       $subsecao = addslashes($_POST['subsecao']);  $_SESSION['life_pesquisa_subsecao'] = $subsecao;   } elseif (isset($_SESSION['life_pesquisa_subsecao'])) {      $subsecao = $_SESSION['life_pesquisa_subsecao'];    }
      if (isset($_POST['item']))     {       $item = addslashes($_POST['item']);          $_SESSION['life_pesquisa_item'] = $item;           } elseif (isset($_SESSION['life_pesquisa_item']))     {      $item = $_SESSION['life_pesquisa_item'];            }
