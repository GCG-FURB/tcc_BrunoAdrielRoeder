<?php
  class ObjetoAprendizagemSegmentInformationTable {
    ...
    public function imprimeFormularioCadastro($eh_informar_segment_information_table, $eh_manter_configuracoes_originais, $cd_segment_information_table, $ds_segment_list, $eh_informar_segment_list, $ds_segmente_group_list, $eh_informar_segmente_group_list) {
      ...
      if ($eh_obrigatorio_segment_list || $eh_obrigatorio_segmente_group_list) {
        $util->linhaComentario('<hr>');
        $util->linhaComentarioChamada('Segmentos');
      
        if ($eh_informar_segment_list == '1') {
          $util->linhaUmCampoTextHint($eh_obrigatorio_segment_list, $conf->retornaDescricaoCampoSegmentInformation_tableListaSegmentos(), $conf->retornaNomeCampoSegmentInformation_tableListaSegmentos(), 'ds_segment_information_table_segment_list', '250', '100', $ds_segment_list, 1);
          $util->campoHidden('nm_segment_information_table_segment_list', $conf->retornaNomeCampoSegmentInformation_tableListaSegmentos());
        } else {
          $util->campoHidden('ds_segment_information_table_segment_list', $ds_segment_list);
        }                        
        if ($eh_informar_segmente_group_list == '1') {
          $util->linhaUmCampoTextHint($eh_obrigatorio_segmente_group_list, $conf->retornaDescricaoCampoSegmentInformation_tableListaSegmentoGrupo(), $conf->retornaNomeCampoSegmentInformation_tableListaSegmentoGrupo(), 'ds_segment_information_table_segmente_group_list', '250', '100', $ds_segmente_group_list, 1);
          $util->campoHidden('nm_segment_information_table_segmente_group_list', $conf->retornaNomeCampoSegmentInformation_tableListaSegmentoGrupo());
        } else {
    ...
    public function imprimeDados($cd_segment_information_table) {
      require_once 'includes/configuracoes.php';                                $conf = new Configuracao();
      ...
      $retorno = '';
      if ($eh_informar_segment_list || $eh_informar_segmente_group_list) {
      $retorno.= "<div class=\"divConteudoUnicoObjetoAprendizagem\">\n";
      $retorno.= "<p class=\"fontConteudoObjetosAprendizagem\">";
      $retorno.= "<b>Segmentos</b>";
      $retorno.= "</p>\n";

      if (($eh_informar_segment_list == '1') && ($ds_segment_list != "")) {
        $retorno.= "<p class=\"fontConteudoDuploObjetosAprendizagem\">";
        $retorno.= "<b>".$conf->retornaInformacaoCampoSegmentInformation_tableListaSegmentos()."</b> ".$ds_segment_list;
        $retorno.= "</p>\n";
      }
      if (($eh_informar_segmente_group_list == '1') && ($ds_segmente_group_list != "")) {
        $retorno.= "<p class=\"fontConteudoDuploObjetosAprendizagem\">";
        $retorno.= "<b>".$conf->retornaInformacaoCampoSegmentInformation_tableListaSegmentoGrupo()."</b> ".$ds_segmente_group_list;
        $retorno.= "</p>\n";
      }
    ...
    public function imprimeDadosRetornoPesquisa($cd_segment_information_table, $tipo) {
      require_once 'includes/configuracoes.php';                                $conf = new Configuracao();
      ...
      $retorno = '';
      if (($eh_informar_segment_list == '1') && ($ds_segment_list != "")) {
        $retorno.= "<b>".$conf->retornaInformacaoCampoSegmentInformation_tableListaSegmentos()."</b> ".$ds_segment_list."<br />\n";
      }
      if (($eh_informar_segmente_group_list == '1') && ($ds_segmente_group_list != "")) {
        $retorno.= "<b>".$conf->retornaInformacaoCampoSegmentInformation_tableListaSegmentoGrupo()."</b> ".$ds_segmente_group_list."<br />\n";
      }
      return $retorno;
    }
