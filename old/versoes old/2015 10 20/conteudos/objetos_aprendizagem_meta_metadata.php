<?php
  class ObjetoAprendizagemMetaMetadata {
    ...
    public function imprimeFormularioCadastro($eh_informar_meta_metadata, $eh_manter_configuracoes_originais, $cd_meta_metadata, $ds_identifier, $eh_informar_identifier, $ds_contribute, $eh_informar_contribute, $ds_metadata_schema, $eh_informar_metadata_schema, $cd_language, $eh_informar_language) {
      ...
      if ($eh_informar_meta_metadata == '1') {
        ...
        if ($eh_informar_identifier == '1') {
          $util->linhaUmCampoTextHint($eh_obrigatorio_identifier, $conf->retornaDescricaoCampoMetaMetadataIdentificador(), 'ds_meta_metadata_identifier', '250', '840', $ds_identifier, 1);
        } else {
          $util->campoHidden('ds_meta_metadata_identifier', $ds_identifier);
        }                                                                                                                 

        if ($eh_informar_contribute == '1') {
          $util->linhaUmCampoTextHint($eh_obrigatorio_contribute, $conf->retornaDescricaoCampoMetaMetadataContribuicao(), 'ds_meta_metadata_contribute', '250', '840', $ds_contribute, 1);
        } else {
          $util->campoHidden('ds_meta_metadata_contribute', $ds_contribute);
        }                                                                                                                 

        if ($eh_informar_metadata_schema == '1') {
          $util->linhaUmCampoTextHint($eh_obrigatorio_metadata_schema, $conf->retornaDescricaoCampoMetaMetadataEsquemaMetadados(), 'ds_meta_metadata_metadata_schema', '250', '840', $ds_metadata_schema, 1);
        } else {
        ...
        if ($eh_informar_language == '1') {        
          ...
          $lin->retornaSeletorLinguagem($cd_language, 'cd_meta_metadata_language', '840', 1, $conf->retornaDescricaoCampoMetaMetadataIdioma());
        } else {
    ...
    public function imprimeDados($cd_meta_metadata, $eh_informar_meta_metadata, $eh_manter_configuracoes_originais, $tipo) {
      require_once 'includes/configuracoes.php';                                $conf = new Configuracao();
      ...
      if ($eh_informar_meta_metadata == '1') {
        if ($eh_informar_identifier == '1') {
          $retorno.= $conf->retornaDescricaoCampoMetaMetadataIdentificador().': '.$ds_identifier.'<br />';
        }                                                                                                                 
        if ($eh_informar_contribute == '1') {
          $retorno.= $conf->retornaDescricaoCampoMetaMetadataContribuicao().': '.$ds_contribute.'<br />';
        }                                                                                                                 
        if ($eh_informar_metadata_schema == '1') {
          $retorno.= $conf->retornaDescricaoCampoMetaMetadataEsquemaMetadados().': '.$ds_metadata_schema.'<br />';
        }                                                                                                                 
        if ($eh_informar_language == '1') {        
          require_once 'conteudos/linguagens.php';                              $lin = new Linguagem();
          $retorno.= $lin->retornaDados($cd_language, $conf->retornaDescricaoCampoMetaMetadataIdioma());
        }
      }
      return $retorno;
    }
