<?php
  class ObjetoAprendizagemTechnicalService {
    ...
    public function imprimeFormularioCadastro($eh_informar_service, $eh_manter_configuracoes_originais, $cd_service, $ds_name, $eh_informar_name, $cd_type, $eh_informar_type, $ds_provides, $eh_informar_provides, $ds_essential, $eh_informar_essential, $ds_protocol, $eh_informar_protocol, $ds_ontology, $eh_informar_ontology, $cd_language, $eh_informar_language, $ds_details, $eh_informar_details) {
      ...
      if ($eh_informar_service == '1') {
        ...
        if ($eh_informar_name == '1') {
          $util->linhaUmCampoTextHint($eh_informar_name, $conf->retornaDescricaoCampoTechnicalServiceNome(), 'ds_service_name', '250', '840', $ds_name, 1);
        } else {
          $util->campoHidden('ds_service_name', $ds_name);
        }
        if ($eh_informar_type == '1') {        
          require_once 'conteudos/tipos.php';                                   $tip = new Tipo();
          $tip->retornaSeletorTipo($cd_type, 'cd_service_type', '840', 1, $conf->retornaDescricaoCampoTechnicalServiceTipo());
        } else {
          $util->campoHidden('cd_service_type', $cd_type);
        }
        if ($eh_informar_provides == '1') {
          $util->linhaUmCampoTextHint($eh_informar_provides, $conf->retornaDescricaoCampoTechnicalServiceFornece(), 'ds_service_provides', '250', '840', $ds_provides, 1);
        } else {
          $util->campoHidden('ds_service_provides', $ds_provides);
        }
        if ($eh_informar_essential == '1') {
          $util->linhaUmCampoTextHint($eh_informar_essential, $conf->retornaDescricaoCampoTechnicalServiceEssencial(), 'ds_service_essential', '250', '840', $ds_essential, 1);
        } else {
          $util->campoHidden('ds_service_essential', $ds_essential);
        }
        if ($eh_informar_protocol == '1') {
          $util->linhaUmCampoTextHint($eh_informar_protocol, $conf->retornaDescricaoCampoTechnicalServiceProtocolo(), 'ds_service_protocol', '250', '840', $ds_protocol, 1);
        } else {
          $util->campoHidden('ds_service_protocol', $ds_protocol);
        }
        if ($eh_informar_ontology == '1') {
          $util->linhaUmCampoTextHint($eh_informar_ontology, $conf->retornaDescricaoCampoTechnicalServiceOntologia(), 'ds_service_ontology', '250', '840', $ds_ontology, 1);
        } else {
        ...
        if ($eh_informar_language == '1') {        
          ...
          $lin->retornaSeletorLinguagem($cd_language, 'cd_service_language', '840', 1, $conf->retornaDescricaoCampoTechnicalServiceIdioma());
        } else {
          $util->campoHidden('cd_service_language', $cd_language);
        }
        if ($eh_informar_details == '1') {
          $util->linhaUmCampoTextHint($eh_informar_details, $conf->retornaDescricaoCampoTechnicalServiceDetalhes(), 'ds_service_details', '250', '840', $ds_details, 1);
        } else {
    ...
    public function imprimeDados($cd_service, $eh_informar_service, $eh_manter_configuracoes_originais, $tipo) {
      require_once 'includes/configuracoes.php';                                $conf = new Configuracao();
      ...
      $retorno = '';
      if ($eh_informar_service == '1') {
        $retorno.= '<b><i>Informações Técnicas - Serviços</i></b><br />';
      
        if ($eh_informar_name == '1') {
          $retorno.= $conf->retornaDescricaoCampoTechnicalServiceNome().': '.$ds_name.'<br />';
        }
        if ($eh_informar_type == '1') {        
          require_once 'conteudos/tipos.php';                                   $tip = new Tipo();
          $retorno.= $tip->retornaDados($cd_type, $conf->retornaDescricaoCampoTechnicalServiceTipo());
        }
        if ($eh_informar_provides == '1') {
          $retorno.= $conf->retornaDescricaoCampoTechnicalServiceFornece().': '.$ds_provides.'<br />';
        }
        if ($eh_informar_essential == '1') {
          $retorno.= $conf->retornaDescricaoCampoTechnicalServiceEssencial().': '.$ds_essential.'<br />';
        }
        if ($eh_informar_protocol == '1') {
          $retorno.= $conf->retornaDescricaoCampoTechnicalServiceProtocolo().': '.$ds_protocol.'<br />';
        }
        if ($eh_informar_ontology == '1') {
          $retorno.= $conf->retornaDescricaoCampoTechnicalServiceOntologia().': '.$ds_ontology.'<br />';
        }
        if ($eh_informar_language == '1') {        
          require_once 'conteudos/linguagens.php';                              $lin = new Linguagem();
          $retorno.= $lin->retornaDados($cd_language, $conf->retornaDescricaoCampoTechnicalServiceIdioma());
        }
        if ($eh_informar_details == '1') {
          $retorno.= $conf->retornaDescricaoCampoTechnicalServiceDetalhes().': '.$ds_details.'<br />';
        }    
      }
      return $retorno;
    }
